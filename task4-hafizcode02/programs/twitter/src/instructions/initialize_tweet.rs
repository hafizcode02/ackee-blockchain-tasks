...
pub fn initialize_tweet(
    ...
    initialized_tweet.topic_length = topic.as_bytes().len() as u8; // Set topic length
    initialized_tweet.tweet_author = ctx.accounts.tweet_authority.key(); // Set author
    initialized_tweet.likes = 0; // Initialize likes
    initialized_tweet.dislikes = 0; // Initialize dislikes
